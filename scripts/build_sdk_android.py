...
def _build_sdk(root_dir, android_submodule_dir, configuration, with_test_lib=False):
    ...
    if with_test_lib:
        ...
        if (configuration == 'release'):
            test_libs_in_dir = '{0}/testlibrary/build/intermediates/intermediate-jars/release'.format(build_dir)
        else:
            test_libs_in_dir = '{0}/testlibrary/build/intermediates/intermediate-jars/debug'.format(build_dir)
