...
def build_example(root_dir, ios_submodule_dir, configuration, app_path):
    # ------------------------------------------------------------------
    # Building AdjustSdk.xcframework file.
    debug_green('Building AdjustSdk.xcframework file ...')
    _build_sdk(root_dir, ios_submodule_dir, configuration, with_test_lib=False)

    # ------------------------------------------------------------------
    # Copying new AdjustSdk.xcframework to example app.
    debug_green('Copying new AdjustSdk.xcframework to example app ...')
    remove_dir_if_exists('{0}/proj.ios_mac/ios/AdjustSdk.xcframework'.format(app_path))
    copy_dir_contents('{0}/libs/ios/AdjustSdk.xcframework'.format(root_dir), '{0}/proj.ios_mac/ios/AdjustSdk.xcframework'.format(app_path), copy_symlinks=True)
...
def build_test(root_dir, ios_submodule_dir, configuration, app_path):
    # ------------------------------------------------------------------
    # Building AdjustSdk.xcframework and AdjustTestLibrary.framework file.
    debug_green('Building AdjustSdk.xcframework and AdjustTestLibrary.framework file ...')
    _build_sdk(root_dir, ios_submodule_dir, configuration, with_test_lib=True)

    # ------------------------------------------------------------------
    # Copying new AdjustSdk.xcframework & AdjustTestLibrary.framework to example app.
    debug_green('Copying new AdjustSdk.xcframework & AdjustTestLibrary.framework to test app ...')
    remove_dir_if_exists('{0}/proj.ios_mac/ios/AdjustSdk.xcframework'.format(app_path))
    ...
    debug_green('Success. Make sure AdjustSdk.xcframework & AdjustTestLibrary.framework is linked in "Build Phases". Run it with Xcode from "proj.ios_mac" directory.')
...
def _build_sdk(root_dir, ios_submodule_dir, configuration, with_test_lib=False):
    ...
    # ------------------------------------------------------------------
    # Deleting old AdjustSdk.xcframework file.
    debug_green('Deleting old AdjustSdk.xcframework file ...')
    remove_dir_if_exists('{0}/AdjustSdk.xcframework'.format(lib_out_dir))
    ...
    # ------------------------------------------------------------------
    # Rebuilding AdjustSdk.xcframework file.
    debug_green('Rebuilding AdjustSdk.xcframework file ...')
    change_dir(build_dir)
    # if configuration == 'debug':
    #     xcode_build('AdjustStatic', 'Debug')
    # else:
    #     xcode_build('AdjustStatic', 'Release')
    execute_command(['./scripts/build_frameworks.sh', '-xs', '-ios', '-tv'])

    # ------------------------------------------------------------------
    # Copying AdjustSdk.xcframework to ${ROOT_DIR}/${LIB_OUT_DIR}.
    debug_green('Copying AdjustSdk.xcframework to {0} ...'.format(lib_out_dir))
    copy_dir_contents('{0}/sdk_distribution/xcframeworks-static/AdjustSdk-iOS-tvOS-xcframework/AdjustSdk.xcframework'.format(build_dir), '{0}/AdjustSdk.xcframework'.format(lib_out_dir), copy_symlinks=True)
