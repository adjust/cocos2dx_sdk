...
cd ${NEW_APP_PATH}/examples
rm -rfv Classes
echo "Success"

echo -e "${GREEN}>>> Replacing Classes directory ${NC}"
cd ${ROOT_DIR}
cp -Rv ${EXAMPLE_DIR}/Classes ${NEW_APP_PATH}/examples
echo "Success"

echo
echo -e "${GREEN}>>> Add the following environmental variable to your .zshrc file or .bashrc: ${NC}"
echo -e "${YELLOW}export COCOS_EXAMPLES_APP=${NEW_APP_PATH}/examples ${NC}"
echo -e "${YELLOW}export PATH=\$COCOS_EXAMPLES_APP:\$PATH ${NC}"
echo
...
echo -e "${GREEN}>>> - Follow the README instructions for adding the necessary permissions to AndroidManifest.xml. ${NC}"
...
echo -e "${GREEN}>>> - Run \`scripts/testing_ios.sh\`. That should copy the necessary files to the examples app. ${NC}"
